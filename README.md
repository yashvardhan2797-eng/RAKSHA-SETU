...
**Vehicle Crash Detection, Impact Analysis & Emergency Response Platform**

RAKSHA SETU (Sanskrit: "bridge of protection") is an end-to-end demo platform that
detects vehicle crashes from ECU/telematics sensor data, scores crash confidence on-board,
alerts a cloud control center, notifies saved emergency contacts, and gives the driver a
**30-second false-alarm cancellation window** — plus a full crash-physics lab for
...
|---|---|---|
| **Web control center** | `artifacts/raksha-setu` | React + Vite dashboard: live incidents, 30-s cancel window, personal info, **Crash Lab** (physics/severity/simulation engines, reports, methodology) |
| **Cloud API** | `artifacts/api-server` | Express + Zod + Drizzle: incident ingest, lifecycle (active → cancelled / escalated), contact dispatch |
| **Vehicle integration** | `vehicle-integration` | The **in-vehicle agent**: simulated ECU/CAN + OEM telematics + GNSS bus (10 Hz), crash-confidence check, alert transmission, driver HMI (`public/index.html`), SSE receiver dashboard (`public/receiver.html`) |
| **Android apps** | `apk-dist/`, `D:\apk-build` (build scripts) | User app (SOS, profile, contacts) + operator dashboard app, ~40 KB each |
| **Ops scripts** | `scripts/` | standalone APK server (`apk-server.mjs`), public publisher (`publish-apk.mjs`), one-click server launcher |
...
- `.freebuff/run.md` — preview/dev-server runbook

## Verified physics

The Crash Lab reproduces the reference case dynamically (never hard-coded):
1500 kg, 80→60 km/h, 0.15 s → ΔV 5.56 m/s · 37.0 m/s² · 3.78 g · 55.6 kN estimated
