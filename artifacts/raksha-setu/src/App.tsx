...
function IncidentDetail({ incident: snapshot, onClose, pushToast }: { incident: Incident; onClose: () => void; pushToast: AppProps['pushToast'] }) {
  ...
  return <div className="fixed inset-0 z-50 flex justify-end"><button data-testid="button-close-incident-overlay" aria-label="Close incident detail overlay" onClick={onClose} className="absolute inset-0 bg-slate-950/30" /><aside data-testid={`drawer-incident-${incident.id}`} className="animate-rise-in relative h-full w-full max-w-xl overflow-y-auto border-l border-slate-200 bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-[10px] font-medium tracking-[.12em] text-cyan-700">{incident.code}</p><h2 className="mt-1 text-lg font-extrabold tracking-tight">{incident.type}</h2><p className="mt-1 text-xs text-slate-500">{incident.location}</p></div><button aria-label="Close incident detail" data-testid="button-close-incident-detail" onClick={onClose} className="rounded-md p-1.5 text-slate-400 hover:bg-slate-100"><X size={16} /></button></div>    <div className="mt-4 flex flex-wrap gap-2"><StatusBadge label={incident.severity} /><StatusBadge label={status} />{incident.cancelledAsFalseAlarm && <StatusBadge label="False alarm" tone="low" />}</div>
    ...
    {incident.owner && <div className="mt-5 rounded-xl border border-red-100 bg-red-50/40 p-4">
      ...
      <div className="mt-2.5 space-y-2">{incident.owner.emergencyContacts.map((contact) => <div key={contact.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-white p-2.5"><div><p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Emergency contact{contact.isPrimary ? ' · PRIMARY' : ''}</p><p className="mt-1 text-xs font-bold text-slate-700">{contact.name} <span className="font-normal text-slate-500">({contact.relation})</span></p></div><a href={`tel:${contact.phone.replace(/[^+\d]/g, '')}`} className="inline-flex items-center gap-1.5 rounded-lg bg-cyan-700 px-3 py-2 text-[10px] font-bold text-white"><Phone size={12} />{contact.phone}</a></div>)}</div>
    </div>}
...
function Overview({ pushToast }: AppProps) {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="font-mono text-[10px] uppercase tracking-[.14em] text-cyan-700">Operations overview</p><h2 className="mt-1 text-base font-extrabold text-slate-800">Live control-center picture</h2><p className="mt-1 text-xs text-slate-500">Real-time emergency response and road safety operations.</p></div><div className="flex items-center gap-3"><div className="text-right"><p className="text-[10px] font-bold uppercase tracking-[.1em] text-slate-400">Last updated</p><p data-testid="text-overview-last-updated" className="mt-0.5 font-mono text-xs font-medium text-slate-700">{lastUpdated}</p></div><button data-testid="button-refresh-overview" onClick={() => { setLastUpdated(`${new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })} IST`); pushToast('Overview mock data refreshed', 'success'); }} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50"><RefreshCw size={14} />Refresh</button></div></div>
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><MetricCard label="Active Incidents" value={active.length.toString().padStart(2, '0')} detail="+2 today" icon={Siren} accent="red" trend="up" /><MetricCard label="Critical Incidents" value={critical.length.toString().padStart(2, '0')} detail="Requires immediate response" icon={Zap} accent="red" trend="down" /><MetricCard label="Vehicles Online" value="124" detail="97% connected" icon={CarFront} accent="blue" trend="up" /><MetricCard label="Response Teams" value="18" detail="15 available" icon={Users} accent="teal" trend="up" /><MetricCard label="Average Response Time" value="08:42" detail="-12% this week" icon={Clock3} accent="amber" trend="up" /></div>
...
function VehicleModal({ vehicle, onClose, onSave }: { vehicle?: Vehicle; onClose: () => void; onSave: (vehicle: Vehicle) => void }) {
  ...
  return <Modal title={vehicle ? 'Edit vehicle' : 'Add vehicle'} onClose={onClose} testId="modal-vehicle"><div className="mt-5 grid gap-4 sm:grid-cols-2">{[['id', 'Vehicle ID'], ['registration', 'Registration Number'], ['type', 'Vehicle Type'], ['model', 'Model'], ['driver', 'Driver'], ['deviceId', 'Device ID']].map(([key, label]) => <label key={key} className="text-xs font-bold text-slate-600">{label}<input value={String(form[key as keyof Vehicle])} onChange={(event) => update(key as keyof Vehicle, event.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs outline-none focus:border-cyan-500" /></label>)}<label className="text-xs font-bold text-slate-600">Status<select value={form.status} onChange={(event) => update('status', event.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs outline-none"><option>ONLINE</option><option>OFFLINE</option><option>MAINTENANCE</option></select></label></div><div className="mt-6 flex justify-end gap-2 border-t border-slate-100 pt-4"><button onClick={onClose} className="rounded-lg border border-slate-200 px-4 py-2.5 text-xs font-bold text-slate-600">Cancel</button><button onClick={() => onSave(form)} className="rounded-lg bg-cyan-700 px-4 py-2.5 text-xs font-bold text-white">Save Vehicle</button></div></Modal>;
}
...
function LineChart({ values, color = '#0e7d8b', suffix = '' }: { values: number[]; color?: string; suffix?: string }) {
  const width = 620;
  const height = 190;
  const max = Math.max(...values);
  const min = Math.min(...values);
  const points = values.map((value, index) => `${(index / (values.length - 1)) * width},${height - ((value - min) / Math.max(1, max - min)) * 145 - 15}`).join(' ');
  return <div className="relative"><svg viewBox={`0 0 ${width} ${height}`} className="h-52 w-full overflow-visible"><line x1="0" y1="175" x2={width} y2="175" stroke="#e2e8f0" /><line x1="0" y1="90" x2={width} y2="90" stroke="#eef2f5" strokeDasharray="4 5" /><polyline fill="none" stroke={color} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" points={points} />{values.map((value, index) => { const [x, y] = points.split(' ')[index].split(','); return <g key={index}><circle cx={x} cy={y} r="4" fill="white" stroke={color} strokeWidth="2" /><title>{value}{suffix}</title></g>; })}</svg><div className="flex justify-between px-1 font-mono text-[9px] text-slate-400"><span>Mon</span><span>Tue</span><span>Wed</span><span>Thu</span><span>Fri</span><span>Sat</span><span>Sun</span></div></div>;
}

function DonutChart({ items }: { items: { label: string; value: number; color: string }[] }) {
  let current = 0;
  const gradient = items.map((item) => { const start = current; current += item.value; return `${item.color} ${start}% ${current}%`; }).join(', ');
  return <div className="flex items-center gap-6"><div className="relative h-36 w-36 shrink-0 rounded-full" style={{ background: `conic-gradient(${gradient})` }}><div className="absolute inset-7 flex items-center justify-center rounded-full bg-white text-center"><span className="font-mono text-lg font-bold text-slate-800">100%<small className="block font-sans text-[9px] font-medium text-slate-400">total</small></span></div></div><div className="space-y-2">{items.map((item) => <div key={item.label} className="flex items-center gap-2 text-[10px] text-slate-600"><i className="h-2 w-2 rounded-full" style={{ backgroundColor: item.color }} />{item.label}<span className="ml-auto font-mono font-bold">{item.value}%</span></div>)}</div></div>;
}

function BarChart({ values, labels, color = '#0e7d8b' }: { values: number[]; labels: string[]; color?: string }) {
  const max = Math.max(...values);
  return <div className="flex h-48 items-end gap-3 border-b border-l border-slate-200 px-4 pb-0 pt-5">{values.map((value, index) => <div key={labels[index]} className="group flex flex-1 flex-col items-center justify-end gap-2"><span className="hidden font-mono text-[9px] text-slate-400 group-hover:block">{value}%</span><div className="w-full rounded-t-sm transition-all group-hover:opacity-80" style={{ height: `${(value / max) * 150}px`, backgroundColor: color }} /><span className="font-mono text-[9px] text-slate-400">{labels[index]}</span></div>)}</div>;
}

function AnalyticsPage() {
  const [range, setRange] = useState('Last 30 days');
  const [, setLocation] = useLocation();
  return <div className="mx-auto max-w-[1500px] space-y-5"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-slate-500">{range === 'Last 30 days' ? '01 Aug — 30 Aug 2026' : '24 Aug — 30 Aug 2026'} · Compared with previous period</p><button onClick={() => setRange(range === 'Last 30 days' ? 'Last 7 days' : 'Last 30 days')} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600"><SlidersHorizontal size={14} />{range}<ChevronDown size={13} /></button></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><MetricCard label="Total Incidents" value="184" detail="+12.4% from previous period" icon={Siren} accent="red" trend="up" /><MetricCard label="Resolved Incidents" value="169" detail="91.8% resolution rate" icon={CheckCircle2} accent="teal" trend="up" /><MetricCard label="Average Response Time" value="08:42" detail="1m 18s faster than target" icon={Clock3} accent="amber" trend="up" /><MetricCard label="Emergency Escalations" value="27" detail="14.7% of all incidents" icon={Zap} accent="red" /><MetricCard label="Driver Safe Rate" value="96.4%" detail="Across active fleet" icon={HeartPulse} accent="blue" trend="up" /></div><div className="grid gap-5 lg:grid-cols-2"><section className="rounded-xl border border-slate-200 bg-white p-5"><SectionHeading eyebrow="Incidents by day" title="Incoming incident volume" /><LineChart values={analyticsData.incidentsByDay} color="#c92f35" /></section><section className="rounded-xl border border-slate-200 bg-white p-5"><SectionHeading eyebrow="Incident types" title="Where response time is spent" /><DonutChart items={analyticsData.incidentTypes} /></section><section className="rounded-xl border border-slate-200 bg-white p-5"><SectionHeading eyebrow="Severity distribution" title="Severity mix" /><BarChart values={analyticsData.severity.map((item) => item.value)} labels={analyticsData.severity.map((item) => item.label.slice(0, 3))} color="#d97706" /></section><section className="rounded-xl border border-slate-200 bg-white p-5"><SectionHeading eyebrow="Average response time" title="Response time by day" /><LineChart values={analyticsData.responseTime} color="#0e7d8b" suffix=" min" /></section><section className="rounded-xl border border-slate-200 bg-white p-5 lg:col-span-2"><SectionHeading eyebrow="Response performance" title="Team performance score" /><BarChart values={analyticsData.performance} labels={['Alpha', 'Bravo', 'Charlie', 'Delta', 'Echo']} color="#2d8a61" /></section><section className="rounded-xl border border-slate-200 bg-white p-5 lg:col-span-2"><SectionHeading eyebrow="Crash lab" title="Crash-simulation analytics" action="Open Crash Simulator" onAction={() => setLocation('/simulator')} /><SimulationAnalyticsSection /></section></div></div>;
}
...
function LoginPage({ onLogin, pushToast }: { onLogin: (user: LoginUser) => void; pushToast: AppProps['pushToast'] }) {
  ...
  return <div className="flex min-h-[100dvh] items-center justify-center bg-[#172634] p-4"><div className="grid w-full max-w-4xl overflow-hidden rounded-2xl border border-[#2d4050] bg-[#1d3141] shadow-2xl lg:grid-cols-2">
    <div className="hidden flex-col justify-between border-r border-[#2d4050] bg-[#172634] p-8 lg:flex">
      <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#f3c44e] text-[#172634]"><Shield size={22} strokeWidth={2.5} /></div><div><p className="text-base font-extrabold tracking-[.12em] text-white">RAKSHA SETU</p><p className="font-mono text-[9px] uppercase tracking-[.16em] text-slate-400">Emergency Response Platform</p></div></div>
      <div><h2 className="text-xl font-extrabold leading-snug text-white">Every second counts.<br />Respond with confidence.</h2><p className="mt-3 text-xs leading-5 text-slate-400">Crash detection, driver verification with a 30-second false-alarm window, live geo-location and medical profiles — one control center for the whole response network.</p>
        <div className="mt-6 space-y-2.5">{[['30s', 'False-alarm cancellation window'], ['GPS', 'Live crash geo-location'], ['24×7', 'Medical profile at the scene']].map(([big, label]) => <div key={big} className="flex items-center gap-3"><span className="w-14 shrink-0 rounded-lg bg-[#f3c44e]/15 px-2 py-1 text-center font-mono text-[11px] font-bold text-[#f3c44e]">{big}</span><span className="text-xs text-slate-300">{label}</span></div>)}</div></div>
    ...
    <form onSubmit={submit} className="bg-white p-8"><h1 className="text-lg font-extrabold tracking-tight text-slate-800">Sign in</h1><p className="mt-1 text-xs text-slate-500">Access the control center dashboard.</p>
      ...
      <button type="submit" data-testid="button-login-submit" className="mt-5 flex w-full items-center justify-center gap-2 rounded-lg bg-cyan-700 px-4 py-3 text-xs font-bold text-white hover:bg-cyan-800"><LogIn size={15} />Sign in to control center</button>
...
function ProfilePage({ pushToast }: AppProps) {
  ...
  return <div className="mx-auto max-w-[1100px] space-y-5">
    <section className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-4"><span className="flex h-12 w-12 items-center justify-center rounded-full bg-[#d9e9eb] text-sm font-extrabold text-cyan-800">{userProfile.fullName.split(' ').map((part) => part[0]).slice(0, 2).join('')}</span><div><h2 className="text-base font-extrabold text-slate-800">{userProfile.fullName}</h2><p className="text-xs text-slate-500">{userProfile.gender} · {userProfile.age} yrs · Blood group {userProfile.bloodGroup}</p></div></div><StatusBadge label={saved ? 'Saved' : 'Unsaved changes'} tone={saved ? 'good' : 'monitoring'} /></div>
      ...
      <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4"><p className="text-[10px] text-slate-400">Shared with responders only when a crash is detected for this vehicle.</p><button data-testid="button-save-profile" onClick={() => { setSaved(true); pushToast('Personal info saved to this browser session', 'success'); }} className="rounded-lg bg-cyan-700 px-4 py-2.5 text-xs font-bold text-white hover:bg-cyan-800">Save personal info</button></div></section>
